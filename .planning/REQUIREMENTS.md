...
- [x] **DISP-02**: User messages longer than 500 characters are truncated with a "Show more" toggle
- [x] **DISP-03**: Thinking blocks, tool calls, and tool results have distinct semantic color tints (purple/amber/green/red)
...
- [x] **TOOL-03**: Bash tool calls display description as label and command in terminal-styled code block
- [x] **TOOL-04**: Tool input rendering dispatches by tool name (Edit->diff, Read->code, Bash->terminal, others->JSON fallback)
...
- [x] **COMP-02**: Compaction boundaries render as amber markers with token delta ("45k -> 12k, 33k freed")
