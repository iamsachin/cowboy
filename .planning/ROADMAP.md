...
- v1.1 Conversation View Polish - Phases 10-13 (in progress)
...
<details>
<summary>v1.0 MVP Phase Details (Phases 1-9) - SHIPPED 2026-03-04</summary>
...
**Goal**: Assistant messages and their associated tool calls are correctly grouped into typed Turn objects, enabling the UI layer to render grouped response blocks
**Depends on**: Phase 9 (v1.0 complete)
**Requirements**: GROUP-01, GROUP-02
**Success Criteria** (what must be TRUE):
  1. Opening a conversation detail view shows assistant turns grouped with their associated tool calls instead of a flat timeline
  2. The main assistant output text is visible on each turn without expanding or clicking anything
  3. Orphaned tool calls (messageId mismatch) are attached to the nearest preceding assistant turn, not silently dropped
  4. Consecutive assistant messages without an intervening user message produce separate turn groups, not merged
**Plans**: 2 plans
...
**Depends on**: Phase 10
**Requirements**: GROUP-03, GROUP-04, GROUP-05, GROUP-06, UX-01
**Success Criteria** (what must be TRUE):
  1. Each collapsed assistant response block shows a summary header with model name, tool call count, duration, and timestamp
  2. User can expand a response block to see thinking content as a collapsible section (using details/summary, not nested DaisyUI collapse)
  3. User can expand a response block to see tool call rows with name, status, and duration, and can further expand individual tool calls to see input/output details
  4. A single toggle button expands or collapses all response blocks on the page
  5. Collapse state persists across Vue re-renders (controlled via reactive Map, not uncontrolled checkbox state)
**Plans**: 3 plans
...
(Note: Phase 13 depends on Phase 11 not Phase 12, so 12 and 13 could run in parallel.)
